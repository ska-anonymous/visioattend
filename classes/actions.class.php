...
class Actions
{
    ...
    {
        foreach ($_POST as $k => $v) {
            if (!is_array($_POST[$k]) && !is_numeric($_POST[$k]) && !empty($_POST[$k]) && $k != 'face_descriptor') {
                $_POST[$k] = addslashes(htmlspecialchars($v));
            }
        }
        extract($_POST);
        ...
        if (!empty($id)) {
            ...
            $sql = "INSERT `students_tbl` set `name` = '{$name}', `class_id` = '{$class_id}', `rollno` = '{$rollno}', `face_descriptor` = '{$face_descriptor}'";
        }
        if ($check->num_rows > 0) {
    ...
    public function list_student($class_id = '0')
    {
        if (!$class_id || $class_id == 0) {
            ...
            $sql = "SELECT `students_tbl`.*, `class_tbl`.`name` as `class` FROM `students_tbl` inner join `class_tbl` on `students_tbl`.`class_id` = `class_tbl`.`id` WHERE `class_tbl`.`id` = $class_id order by `students_tbl`.`name` ASC";
        }
        $qry = $this->conn->query($sql);
        return $qry->fetch_all(MYSQLI_ASSOC);
    }
    ...
    public function change_password()
    {
        ...
        // first check new and confirm passwords are same or not
        if ($newpass != $confirmpass) {
            return ['status' => 'error', 'msg' => 'New password and Confirm password do not match'];
        }

        $old_password_hash = md5($oldpass);
        $new_password_hash = md5($newpass);

        $user_id = $_SESSION['user_data']['id'];
        // check whether old password is correct or not
        $check_old_pass = $this->conn->query("SELECT * FROM users_tbl WHERE `id` = '$user_id' AND `password` = '$old_password_hash'");

        if ($check_old_pass->num_rows == 0) {
            return ['status' => 'error', 'msg' => 'Old password is wrong'];
        }

        try {
            $change_password = $this->conn->query("UPDATE users_tbl SET `password` = '$new_password_hash' WHERE `id` = '$user_id'");
            return ["status" => "success"];
        } catch (Exception $e) {
